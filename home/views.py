...
plotTypes = ['Scatter', 'Grouped Bar Chart']
plotMap = {plotTypes[0]: 'scatter()', plotTypes[1]: 'groupedBarChart()'}
...
def home(request):
    ...
    if request.GET.get('addSector') is not None:
        if counterSector != 4:
            counterSector = counterSector + 1
            counterSectorArray.append(counterSector)
    if request.GET.get('removeSector') is not None:
        if counterSector != 1:
    ...
    if 2 in counterSectorArray:
        ...
        if request.GET.get('sectors2') is not None:
    ...
    if 4 in counterSectorArray:
        ...
        if request.GET.get('sectors4') is not None:
    ...
    getSelectedPlot = plotMap[request.GET.get('plots')]
    context = {
        'plot': eval(getSelectedPlot),
        'plot': lineChart(),
        'sectors': sectors,
